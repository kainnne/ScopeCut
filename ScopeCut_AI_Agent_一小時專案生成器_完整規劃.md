...
- ScopeCut 面向有想法與熱誠、但不熟悉 AI Agent 的使用者；重點是用 UI 協助系統化思考，再產生可直接交給 Agent 的第一版 Project Prompt。
- 主要問題改用大量但有整理的可選項，依「產品與工具、內容與表達、社群與服務」等類別收合；每一步仍保留自由輸入。
- 「執行條件」不再拆成工具、限制與排除項目，只保留一個「其他備註」欄位並放入 Prompt。
- 「素材與參考」將實作真正的附件上傳。檔案規模不以任意縮短輸出或過小的 MB 上限處理，而是在送出前依實際 input tokens 預估成本；大型檔案改以檢索方式讀取，檔案短期保存並在完成後刪除。
...
## 測試階段成本、點數與用量 Dashboard 決策（2026-08-24）
...
- 五分鐘是完成時間目標，不是內容長度限制。請求採背景執行並記錄延遲；接近五分鐘仍未完成時取消或標記 timeout，不讓模型在背景無限繼續花費。
...
- 附件內容只在生成所需期間保存；用量資料庫只保留檔名雜湊、類型、大小、頁數與 token 數，不長期保存原始文件或 Project Prompt。

### 模擬點數

- 測試階段不收費、不要求一般使用者登入；瀏覽器取得一個簽署過的匿名識別碼，用於顯示該瀏覽器的模擬點數與用量紀錄。
- 一點代表一個內部成本級距，暫以預估 US$0.03 為基準。使用者只看到「預估需要幾點」與「實際使用幾點」，不看到美元、模型單價或真實 token 費用。
- 送出前依合理的預估輸出計算模擬點數；完成後依實際成本重新記錄。若需要兩點以上，先提示原因通常來自附件較多、文件較長或選擇較高模型，再由使用者以模擬點數確認繼續。
- 測試點數不接付款。點數不足時提供「使用測試額度繼續」或要求稍後再試，用來測試未來付費流程的 UI 與資料結構。
...
- 不建立三份會互相不同步的實體資料庫。Cloudflare D1 保存一份原始 usage events 與每日彙總，再由三組權限不同的 API／Dashboard 呈現。
- 公開 Dashboard：任何人可看，只顯示匿名化的生成份數、模擬點數、服務狀態與成本級距分布；不得顯示美元、真實 token、Email、匿名識別碼、檔名或 Prompt。
- 個人 Dashboard：不登入，以簽署的匿名瀏覽器識別碼讀取；顯示自己的剩餘模擬點數、每日生成次數、每次使用點數、狀態與時間。清除瀏覽器資料或換裝置後不保證能找回紀錄。
- 管理 Dashboard：只有 Kainnne 管理者可看，顯示所有請求的 input／cached／reasoning／output tokens、美元成本、模型、工具費、延遲、附件規模、成本分級、失敗原因、每日預算與保留額度；可展開匿名帳號查看個別紀錄，但不預設保存 Prompt 原文。
- 管理 Dashboard 必須另外使用既有 Kainnne Email OTP 或 Cloudflare Access 保護；「測試階段不用登入」只適用於一般生成與個人匿名用量頁，不代表真實美元資料可以公開。

### 建議資料表

- `anonymous_users`：匿名使用者 ID、建立時間、最後使用時間；不保存明文 IP。
- `usage_events`：每次請求的狀態、模型、token 明細、預估與實際成本、點數、延遲、附件統計、讀取模式與錯誤原因。
- `usage_files`：檔案類型、位元組、頁數、token 數與雜湊，不保存原始附件內容。
- `daily_user_usage`：匿名使用者每日生成次數、模擬點數與各成本級距次數。
- `daily_system_usage`：每日總請求、成功／失敗／timeout、token、真實美元成本、保留額度、點數、各成本級距、平均／最高成本與延遲統計。
- `runtime_config`：每日美元預算、點數換算、分級門檻、預設模型、reasoning 與功能開關，方便管理者調整而不必重新部署。

### 每日更新與排程

- 每次 API 請求都要即時寫入 usage event 並更新當日預算，成本防護不能依賴個人電腦是否開機。
- Dashboard 的即時資料直接由 D1 查詢；未來額外加入的「每日報告／快照」才由本機排程產生。
- 本機排程只在電腦開啟且進入指定時間窗時執行，不補跑錯過的日期；電腦沒開時當天不產生報告，但不影響伺服器端的即時用量紀錄與 US$3 預算保護。

### 第一版實作狀態（2026-08-24）

- 已在本機完成匿名測試 session；一般生成不要求 Email，管理 Dashboard 仍使用指定 Kainnne Email OTP。
- 已完成附件上傳、`/responses/input_tokens` 精確估算、模擬點數確認、100K／272K 路由與大型附件 File Search 草稿。
- 已完成最壞成本保留、每日 US$3 原子性門檻、背景生成、五分鐘 timeout，以及依 Responses API 實際 usage 結算與釋放差額。
- 已完成 `usage_events`、`usage_files`、`daily_user_usage`、`daily_system_usage`、`anonymous_users` 與 `runtime_config` D1 schema。
